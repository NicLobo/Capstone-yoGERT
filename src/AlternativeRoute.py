...
class AlternativeRoute:
    ...
    #  @param listOfStops list of Point type consisting of the stop points GPS coordinates for the trace. 
    ...
    def __init__(self, listOfStops, optimizer = "length"):
        try:
            if optimizer not in ["time", "length"]:
                ...
                self.network = NetworkGraph(listOfStops, "bike")
                self.path = ShortestRouteTrace(self.network, listOfStops, optimizer)
